...
let
  ...
  patchOpenssl = pkgs:
    if pkgs.stdenv.hostPlatform.libc == "musl" then
      pkgs.openssl.override { static = true; }
    else if pkgs.stdenv.hostPlatform == pkgs.stdenv.buildPlatform then
      pkgs.openssl
    else
      ...
      setupHook = buildPackages.writeText "exports.sh" ''
        ${name}-setup-env() {
          ${
            ...
            ({ name, value }: "export ${name}=${lib.escapeShellArg value}") envs
          }
        }
...
in rec {
  patches = {
    inherit patchOpenssl patchCurl patchPostgresql joinOpenssl propagateEnv;
  };
  ...
  capLints = makeOverride {
    ...
    overrideArgs = old: {
      rustcLinkFlags = old.rustcLinkFlags or [ ] ++ [ "--cap-lints" "warn" ];
    };
  };

  cc = if pkgs.stdenv.hostPlatform.isDarwin then
    makeOverride {
      name = "cc";
      overrideAttrs = drv: {
  ...
  fsevent-sys = if pkgs.stdenv.hostPlatform.isDarwin then
    makeOverride {
      name = "fsevent-sys";
      overrideAttrs = drv: {
        propagatedBuildInputs = drv.propagatedBuildInputs or [ ]
  ...
  reqwest = if pkgs.stdenv.hostPlatform.isDarwin then
    makeOverride {
      name = "reqwest";
      overrideAttrs = drv: {
        propagatedBuildInputs = drv.propagatedBuildInputs or [ ]
  ...
  libdbus-sys = pkgs.rustBuilder.rustLib.makeOverride {
    name = "libdbus-sys";
    overrideAttrs = drv: {
      propagatedBuildInputs = drv.propagatedBuildInputs or [ ] ++ [ pkgs.dbus ];
    };
  };
  ...
  libudev-sys = pkgs.rustBuilder.rustLib.makeOverride {
    name = "libudev-sys";
    overrideAttrs = drv: {
      propagatedBuildInputs = drv.propagatedBuildInputs or [ ] ++ [ pkgs.udev ];
      buildPhase = ''
  ...
  libgit2-sys = if pkgs.stdenv.hostPlatform.isDarwin then
    makeOverride {
      name = "libgit2-sys";
      overrideAttrs = drv: {
        propagatedBuildInputs = drv.propagatedBuildInputs or [ ] ++ [
  ...
  libssh2-sys = makeOverride {
    ...
    overrideAttrs = drv: {
      propagatedNativeBuildInputs = drv.propagatedNativeBuildInputs or [ ]
        ++ [ pkgs.openssl.dev pkgs.zlib.dev ];
    };
  };
    ...
    overrideAttrs = drv: {
      propagatedBuildInputs = drv.propagatedBuildInputs or [ ] ++ [
        (propagateEnv "openssl-sys" [
          {
            name = "RUSTFLAGS";
            value = "--cfg ossl111 --cfg ossl110 --cfg ossl101";
          }
          {
            name = "${
                envize
                (pkgs.rustBuilder.rustLib.rustTriple pkgs.stdenv.buildPlatform)
              }_OPENSSL_DIR";
            value = joinOpenssl (patchOpenssl pkgs.buildPackages);
          }
          {
            name = "${
                envize
                (pkgs.rustBuilder.rustLib.rustTriple pkgs.stdenv.hostPlatform)
              }_OPENSSL_DIR";
            value = joinOpenssl (patchOpenssl pkgs);
          }
          {
  ...
  pq-sys = let
    binEcho = s:
      "${
        pkgs.buildPackages.writeShellScriptBin "bin-echo" "echo ${s}"
      }/bin/bin-echo";
  in makeOverride {
    name = "pq-sys";
    overrideAttrs = drv: {
      ...
      propagatedBuildInputs = drv.propagatedBuildInputs or [ ] ++ [
        (propagateEnv "pq-sys" [
          {
            name = "PG_CONFIG_${envize pkgs.stdenv.buildPlatform.config}";
            value = binEcho "${(patchPostgresql pkgs.buildPackages).lib}/lib";
          }
          {
            name = "PG_CONFIG_${envize pkgs.stdenv.hostPlatform.config}";
            value = binEcho "${(patchPostgresql pkgs).lib}/lib";
          }
        ])
      ];
    };
    ...
    overrideAttrs = drv: {
      propagatedBuildInputs = drv.propagatedBuildInputs or [ ] ++ [
        (propagateEnv "prost-build" [{
          name = "PROTOC";
          value = "${pkgs.buildPackages.buildPackages.protobuf}/bin/protoc";
        }])
      ];
    };
  };
  ...
  rand = if pkgs.stdenv.hostPlatform.isDarwin then
    makeOverride {
      name = "rand";
      overrideAttrs = drv: {
        propagatedBuildInputs = drv.propagatedBuildInputs or [ ]
  ...
  rand_os = if pkgs.stdenv.hostPlatform.isDarwin then
    makeOverride {
      name = "rand_os";
      overrideAttrs = drv: {
        propagatedBuildInputs = drv.propagatedBuildInputs or [ ]
  ...
  ring = if pkgs.stdenv.hostPlatform.isDarwin then
    makeOverride {
      name = "ring";
      overrideAttrs = drv: {
        propagatedBuildInputs = drv.propagatedBuildInputs or [ ]
